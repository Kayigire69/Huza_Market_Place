...
/**
 * Single clear sell path for farmers. Deal type (direct vs commission) is set by Huza ops,
 * not chosen here — both old buttons previously pointed at the same submit URL.
 */
      ...
      <FarmerPanel className="border-[var(--huza-green)]/40 bg-[var(--huza-mint)]/25">
        <div className="flex items-start gap-3">
          <span className="rounded-xl bg-white p-2.5 shadow-sm">
            <ShoppingCart className="size-6 text-[var(--huza-green-dark)]" aria-hidden />
          </span>
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--huza-green-dark)]">
              {t("sellHowItWorksLabel")}
            </p>
            <h2 className="mt-1 text-xl font-bold text-[var(--huza-ink)]">{t("sellDirectTitle")}</h2>
          </div>
        </div>
        <p className="mt-4 text-sm leading-relaxed text-[var(--huza-muted)]">{t("sellUnifiedBody")}</p>
        ...
        <p className="mt-3 text-xs text-[var(--huza-muted)]">{t("sellUnifiedPayNote")}</p>
        <div className="mt-5 flex flex-col gap-2 sm:flex-row">
          ...
          >
            {t("sellCtaSubmitHarvest")} <ArrowRight className="size-4" aria-hidden />
          </Link>
          <Link
            href="/farmer/sales"
